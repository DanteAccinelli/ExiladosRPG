...
StateManager
 ...
 - ChangeState(GameState newState), GetCurrentState(), UpdateState()

GameState
 - enum -> MainMenu, Playing, Combat, Inventory, Dialogue, GameOver
...
AggressiveBehavior : IAIBehavior
 - DecideAction(Enemy enemy, GameContext context)
 - //More Behavior types
...
SpikeTrap : ITrap
 ...
 - SpikeTrap(bool isHidden, int damage)
 - Trigger(GameContext context), Detect(GameContext context)
 - //More Trap types
...
EconomyController
 ...
 - bool CanBuy(Player player, Item item), bool CanSell(Player player, Item item)
 - Buy(Player player, Item item, Shop shop), Sell(Player player, Item item, Shop shop)
 - int GetFinalBuyPrice(Item item), GetFinalSellPrice(Item item)
...
RequirementGroup
 - List<IRequirement> Requirements, bool RequireAll
